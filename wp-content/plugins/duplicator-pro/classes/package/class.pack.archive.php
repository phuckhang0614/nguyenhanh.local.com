...
require_once(DUPLICATOR____PATH . '/lib/forceutf8/src/Encoding.php');
...
//FILTER: CORE ITMES
        ...
        foreach ($storages as $storage) {
            if ($storage->storage_type == DUP_PRO_Storage_Types::Local && $storage->local_filter_protection) {
                $path = SnapIO::safePath($storage->local_storage_folder);
                $realPath = SnapIO::safePath($storage->local_storage_folder, true);
...
//PHP 5 on windows decode patch
    ...
    public static function getDefaultGlobalDirFilter()
    {
        ...
        if (is_null($dirFiltersLits)) {
            ...
            $dirFiltersLits = array(
                ...
                $paths['wpcontent'] . '/wpvividbackups',
    ...
    private function addToDirList($dirPath, $relativePath, $size, $nodes)
    {
        ...
        if (strlen($relativePath) && !DUP_PRO_Secure_Global_Entity::getInstance()->skip_archive_scan) {
            ...
            if ($invalid_name) {
                if ($this->global->getBuildMode() != DUP_PRO_Archive_Build_Mode::Shell_Exec) {
...
// never warings for cores files
    ...
    public static function getOriginalUrls($urlKey = null)
    {
        ...
        if (is_null($origUrls)) {
            ...
            if (DUP_PRO_Global_Entity::get_instance()->homepath_as_abspath) {
                $homeUrl   = site_url();
            } else {
                $homeUrl   = home_url();
